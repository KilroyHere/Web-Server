...
async function createGrid(rows, cols) {
  ...
  artGrid = storedGrid
    ? storedGrid
    : new Array(rows).fill().map(() => new Array(cols));
  ...
  for (let i = 0; i < rows; i++) {
    ...
    for (let j = 0; j < cols; j++) {
      ...
      col.style.backgroundColor = artGrid[i][j]
        ? artGrid[i][j].color
        : "transparent";
...
function handleMouseDown() {
  if (drawCount >= MAX_DRAWINGS) {
    if (Date.now() - firstDrawTime < DRAWING_INTERVAL) {
      alert(
        `You can only draw ${MAX_DRAWINGS} times per minute. Please wait a moment.`
      );
      return;
    } else {
      drawCount = 0;
...
gridButton.addEventListener("click", () => createGrid(100, 100));
...
function deleteGrid2() {
  ...
  fetch(url, options)
    .then(response => {
      ...
      console.log('Request successful'); // Replace the callback with console.log
    })
    .catch(error => {
      // Handle any errors
      console.error(error);
...
async function fetchDataAndCreateGrid() {
  await createGrid(100, 100);
  // Additional code or actions after creating the grid
}
